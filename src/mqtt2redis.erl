...
on_message_publish(Message, Env) ->
    ...
    {ok, SchemaNo} = application:get_env(mqtt2redis, schemacount),
    {ok, TableNo} = application:get_env(mqtt2redis, tablecount),
    {ok, TablePre} = application:get_env(mqtt2redis, tablepre),
    {ok, TablePost} = application:get_env(mqtt2redis, tablepost),    
      ...
      true -> 
        case (length(Topic) >= SchemaNo)  and (length(Topic) >= TableNo) of 
          true  -> 
            ...
            Headers = string:join(["" ++ mqtt2redis:cts(X) ++ "" || X <- HeadersBin], ","),
            Values = string:join(["'" ++ mqtt2redis:cts(X) ++ "'" || X <- lists:map( fun(HeaderBin) -> maps:get(HeaderBin, PayloadMap) end,HeadersBin)], ","), 
            Query = io_lib:format("INSERT INTO  ~s.~s (~s) VALUES(~s) ~n", [mqtt2redis:cts(lists:nth(SchemaNo,Topic)), Table, Headers, Values] ),
            io:format("Query - ~s~n", [Query]),
            mqtt2redis:write(list_to_atom(lists:flatten(io_lib:format("connection_~p", [rand:uniform(10)]))), Query, Env);
          false ->
...
write(NamePid, Query,Env) ->
  io:format("Name of Connection -  ~p~n", [NamePid]), 
    case whereis(NamePid) of 
        undefined ->
            case mqtt2redis:connect(NamePid,Env)  of 
                {ok,Pid} ->
                    write(NamePid,Query,Env);
                {Reason} ->
                    {error, Reason}
...
connectPid(Env) ->
  ...
  {ok, PidNames } = application:get_env(mqtt2redis, noofcon),
  
  lists:map(
    fun(NamePid) ->
      case whereis(NamePid) of 
          undefined ->
            mqtt2redis:connect(NamePid,Env)
      end
    end,
    PidNames
  ).
...
connect(NamePid,Env) ->
  {ok ,Host} = application:get_env(mqtt2redis, host), 
  {ok ,Port} = application:get_env(mqtt2redis, port),
  {ok ,Username} = application:get_env(mqtt2redis, username),
  {ok ,Password} = application:get_env(mqtt2redis, password),
  {ok, Dbname} = application:get_env(mqtt2redis, dbname),
